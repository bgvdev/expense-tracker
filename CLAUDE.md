...
- **No CI gate** — there is no automated lint/test check on PRs. Run `./vendor/bin/pint --test` and `php artisan test` (backend) / `npm run lint` + `npm run build` (frontend) locally before pushing. Note the Docker containers create `frontend/node_modules` and `backend/vendor` as root, so these may need to run *inside* the containers (`docker compose exec web npm run lint`).
