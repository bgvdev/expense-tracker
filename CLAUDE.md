...
- **No CI gate** — there is no automated lint/test check on PRs. Run `./vendor/bin/pint --test` and `php artisan test` (backend) / `npm run lint` + `npm run build` (frontend) locally before pushing.
  - **The test suite runs on Postgres, in a separate `<DB_DATABASE>_test` database** — same engine as production, so constraint-backed unique indexes, unindexed FKs and non-sargable date filters are actually exercised. It is created by `docker/postgres/init/01-create-test-database.sql` on a fresh volume; on an existing one, `docker compose exec db psql -U laravel -d expense_tracker -c 'CREATE DATABASE expense_tracker_test OWNER laravel'`.
  - The connection is pinned by `backend/tests/bootstrap.php`, **not** by `phpunit.xml` alone. PHPUnit's `<env force="true">` does not set `$_SERVER`, which is where Laravel's `env()` looks first, so the `DB_*` vars that `docker-compose.yml` exports into the `api` container win — that is how `RefreshDatabase` once dropped every table in the local dev database. `Tests\TestCase::setUpTraits()` additionally refuses to run against any database whose name does not end in `_test`, before the traits migrate anything. Do not remove either guard.
  - Never run `npm run build` inside the running `web` container: it overwrites the dev server's `.next/` and leaves the app serving 404s for its chunks. Use `docker compose run --rm --no-deps -e BACKEND_URL=http://api:8000 web npm run build`.
...
php artisan test                   # run all tests (Postgres, <DB_DATABASE>_test)
